(function () {
  ...
  var loadFullFonts = function () {
    ...
    var promises = [
      loadSingleFont(roboto),
      loadSingleFont(robotoBold),
      loadSingleFont(robotoCondensed),
      loadSingleFont(robotoItalic),
    ];
  ...
  /* Load full fonts even if the subset fails. */
  Promise.all([ robotoSubset.load(), ]).catch(function (reason) {
    if (reason) {
      console.error(reason);
      loadFullFonts();
      return Promise.reject();
