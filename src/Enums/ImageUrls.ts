export enum ImageUrls {
  AboutHero            = 'http://www.ucl.ac.uk/european-institute/events/2016-17/Ocean.png',
  CoreTeamPicture      = 'https://s3.eu-central-1.amazonaws.com/hellox/images/core+team+ice-9+DT.jpg',
  CreativeTeamPicture  = 'https://s3.eu-central-1.amazonaws.com/hellox/images/Creative+team+DT.jpg',
  FacebookIcon         = 'https://s3.amazonaws.com/x50/images/Facebook.svg',
  HamburgerMenuIcon    = 'https://s3.amazonaws.com/x50/images/HelloX_Burger_2x.svg',
  InstagramIcon        = 'https://s3.amazonaws.com/x50/images/Instagram.svg',
  MainLogo             = 'https://s3.amazonaws.com/x50/images/HelloX_Logo_Landing_Desktop.svg',
  PodcastIcon          = 'https://s3.eu-central-1.amazonaws.com/hellox/images/Play_gold.svg',
  QuoteIcon            = 'https://s3.amazonaws.com/x50/images/HelloX_Quote_1x.svg',
  ReadDiscussIcon      = 'https://s3.eu-central-1.amazonaws.com/hellox/images/Forum_gold.svg',
  ScienceTeamPicture   = 'https://s3.eu-central-1.amazonaws.com/hellox/images/Science+teamDT.jpg',
  SmallLogo            = 'https://s3.amazonaws.com/x50/images/HelloX_Logo_Small_Menu.svg',

  /* Sponsor icons */
  SponsorFramIcon      = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/FRAM.svg',
  SponsorFrittOrdIcon  = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/Fritt_Ord.svg',
  SponsorKommuneIcon   = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/Kommune.svg',
  SponsorKulturradIcon = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/kulturrad.svg',
  SponsorNnkmIcon      = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/NNKM.svg',
  SponsorPolariaIcon   = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/polaria.svg',
  SponsorSparebankIcon = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/Sparebank.svg',
  SponsorUitIcon       = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/UiT.svg',
  SponsorVsfIcon       = 'https://s3.eu-central-1.amazonaws.com/hellox/images/icons/VSF.svg',

  TwitterIcon          = 'https://s3.amazonaws.com/x50/images/Twitter.svg',
  WriteIcon            = 'https://s3.eu-central-1.amazonaws.com/hellox/images/Write_gold.svg',
}

export default ImageUrls;